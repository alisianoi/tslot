...
from src.common.logger import logged, logger
from srv_color.service.color import MyColorService
from srv_color.widget.combo_box import MyComboBox
from srv_color.widget.label import MyLabel
from srv_color.widget.line_edit import MyLineEdit
from srv_color.widget.push_button import MyPushButton
from srv_color.widget.spin_box import MySpinBox


class MyColorSettings(QWidget):
    ...
    def handle_clicked(self):
        logger.debug(f'Here are MyStyleSettings({self.height()}, {self.width()})')
        logger.debug(f'Here is the MyLabel({self.widget_0.height()}, {self.widget_0.width()})')
        logger.debug(f'Here is the MyComboBox({self.widget_1.height()}, {self.widget_1.width()})')
        logger.debug(f'Here is the MyLineEdit({self.widget_2.height()}, {self.widget_2.width()})')
        logger.debug(f'Here is the QSpinBox({self.widget_3.height()}, {self.widget_3.width()})')
        logger.debug(f'Here is the MyPushButton({self.widget_4.height()}, {self.widget_4.width()})')
    ...
    def handle_combo_box_current_index_changed(self, index):
        # logger.debug(f'The index is {index}')
        pass
